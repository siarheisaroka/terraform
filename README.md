...
## TASK 1 - Creating VPC
Change current directory  to `~/tf-lab/base`
...
Ensure that the current directory is `~/tf-lab/base`
...
-	Create `storage.tf`. Name your bucket "gcp-tf-lab-${random_string.my_numbers.result}" to provide it with partition unique name. See [random_string](https://registry.terraform.io/providers/hashicorp/random/latest/docs/resources/string) documentation for details.
...
-	Firewall rule (`name=ssh-inbound`, `port=22`, `allowed_ip_ranges="your_IP or office-IP_ranges"`, `description="allows ssh access from safe IP-range"`, `target_tags=web-instances`).
...
Ensure that current directory is  `~/tf-lab/base`
...
! Change the current directory to  `~/tf-lab/compute`
! Copy `root.tf` from `~/tf-lab/base` to `~/tf-lab/compute`
...
- Create an `google_compute_region_instance_group_manager` resource:
  - `name=gcp-tf-lab-{region}`,
  - `target_size=1`,
  - `instance_template=gcp-tf-lab-{region}`
